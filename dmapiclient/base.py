...
class BaseAPIClient(object):
    ...
    def _request(self, method, url, data=None, params=None, *, wait_for_response=True):
        if not self._enabled:
            return None

        early_abort_exception = self._WontHappenException if wait_for_response else requests.exceptions.ReadTimeout
        ...
        try:
            response = self._requests_retry_session().request(
                ...
                headers=ci_headers,
                json=data,
                timeout=self.timeout if wait_for_response else self.nowait_timeout,
            )
